function [DROIx,DROIy,Center,theta] = VesselROI(I)
...
%
%   [DROIx, DROIy, Center, theta] = VESSELROI(I) gets the image of
%   the ultrasound screen and returns user selected positions of the ROI x
%   and y coordinates, the center of the vessel, and the vessel wall angle. 
%
...
while VesselCheck == 0
    ...
    imagesc(ICen), axis image, colormap jet, axis off;title('Select center of vessel.');
    ...
    hold on
    imagesc(ICen), axis image, colormap jet, axis off; title('Select 2 points on the vessel wall to define the angle');
    [X,Y] = ginputc(2,'Color','r','LineWidth',2);
    X=floor(X);
    Y=floor(Y);
    AngleOpp = X(1) - X(2);
